...
static int add_sw_filtering_rule_element(struct pf_ring_socket *pfr, sw_filtering_rule_element *rule)
{
  ...
  list_for_each_prev(ptr, &pfr->sw_filtering_rules) {
    ...
    //printk("iterating rules, entry=%d, rule-to-add=%d \n",entry->rule.rule_id, rule->rule.rule_id);
    //printk("-EEXIST \n");
    ...
    if(entry->rule.rule_id < rule->rule.rule_id) {
    	//printk("ERROR entry->rule.rule_id < rule->rule.rule_id \n");
    	break;
    }
  ...
  /*if(rule->rule.rule_id != 1000 || rule->rule.rule_id == 0 ) {
         printk("EXIT EXIT \n");
         return(-EEXIST);
         }*/
			 ...
			 int displ, u_int *last_matched_plugin)
{
  ...
  list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules) {
	...
	*fwd_pkt = 1;
	...
	/* SIpFish Start */
	 if(plugin_registration[*last_matched_plugin] != NULL && plugin_registration[*last_matched_plugin]->pfring_plugin_add_rule_multi != NULL) {


	   		struct list_head *pos, *q;
	   		struct rules_container_t *__rule;
	   		sw_filtering_rule_element *swr ;
	   		struct rules_container_t rules_container;
	   		INIT_LIST_HEAD(&rules_container.list);
	   		...
	   		list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules) {
				...
				if(unlikely(enable_debug)) {
					printk("RULE: %d, TMP rule id %d \n",free_rule_element_id,tmp_entry->rule.rule_id );
				}

				if(tmp_entry->rule.rule_id == free_rule_element_id) {
					free_rule_element_id++;
					//printk("incerase counter RULE: %d \n",free_rule_element_id );
				}
				else { break; } /* we found an hole */
			}

	   		rc = plugin_registration[*last_matched_plugin]->pfring_plugin_add_rule_multi(
	   		...
	   		if(rc == 0) {
	   			...
	   			int it = 0;
	   			list_for_each_safe(pos, q, &rules_container.list) {

	   				swr = (sw_filtering_rule_element*) kcalloc(1, sizeof(sw_filtering_rule_element), GFP_ATOMIC);
	   				INIT_LIST_HEAD(&swr->list);

	   		    	__rule = list_entry(pos, struct rules_container_t, list);

	   		    	swr->rule.rule_id 			= (u_int16_t) 						__rule->rule.rule.rule_id;
	   		    	swr->rule.rule_action 		= (rule_action_behaviour) 			__rule->rule.rule.rule_action;
	   		    	swr->rule.locked 			= (u_int8_t) 						__rule->rule.rule.locked;
	   		    	swr->rule.bidirectional 	= (u_int8_t) 						__rule->rule.rule.bidirectional;
	   		    	swr->rule.core_fields 		= (filtering_rule_core_fields) 		__rule->rule.rule.core_fields;
	   		    	swr->rule.plugin_action		= (filtering_rule_plugin_action)	__rule->rule.rule.plugin_action;
	   		    	swr->rule.extended_fields	= (filtering_rule_extended_fields)	__rule->rule.rule.extended_fields;



	   		    	if(unlikely(enable_debug)) {
	   		    		printk("adding dynamic rule to ring, Rule ID=%d \n", swr->rule.rule_id );

	   		    		/*u_int32_t ip;
	   		    		ip = ntohl(swr->rule.core_fields.shost.v4);
	   		    		unsigned char *ip1 = &ip;
	   		    		printk("call->content.deep.peer_a_l7.ip.v4: %d.%d.%d.%d [ DECIMAL ]   \n",ip1[0],ip1[1],ip1[2],ip1[3] );
						*/
	   		    	}
	   		    	write_lock(&pfr->ring_rules_lock);
	   		    	rc = add_sw_filtering_rule_element(pfr, swr);
					write_unlock(&pfr->ring_rules_lock);
	   		    	if(rc != 0) {
	   		    		printk("!!!!!! Could not add rule %d RC WAS %d\n", swr->rule.rule_id, rc );

	   		    	}

	   		    }
	   			/*
	   			 *  DO NOT free swr as its freed later
	   			 *  by del_rule function, rules_container is part of this funcrion's stack, no need to worry.
	   			 */



	   		} else {
	   			 printk("Did not add multi rule because rc was not 0 !!! which means plugin_add_multi_rule_failed - BAD BAD BAD!, this is a bug. \n");
	   		 }
	...
	/* SipFish end */
	 /*
	 	 *  We are adding Else if to the regular pfring_plugin_add_rule or else the result will be us
	 	 *  adding a wildcard rule that will most likley bring all garbage traffic to user space!!!
	 	 * 	very import incase we implement this in a new pfring version!!!!
	 	 *  in a case that this happens no other rules will get evaluated because
	 	 *  we will be allowing all packets once this rule is added @avi jan 31st 2014!
	 	 */
	 	/* SipFish end */



	 else if(*last_matched_plugin
	...
	*fwd_pkt = 1;
	...
	if(*last_matched_plugin
				   && plugin_registration[*last_matched_plugin] != NULL
				   && plugin_registration[*last_matched_plugin]->pfring_plugin_del_rule_multi != NULL)
		{

			 int lrc;
			 lrc = 0;
			 u_int16_t *rules_to_delete = NULL;
			 rules_to_delete = (u_int16_t *)kcalloc(1, sizeof(u_int16_t),GFP_KERNEL);

			 rc = plugin_registration[*last_matched_plugin]->pfring_plugin_del_rule_multi(
		          ...
		          if(rc > 0) {
		        	  ...
		        	  for(i = 0; i < rule_element_id; i++) {
		        		  //printk("22222delete multi rule LOOP %d \n",rules_to_delete[i]);
		        		  write_lock(&pfr->ring_rules_lock);
		        		  remove_sw_filtering_rule_element(pfr, rules_to_delete[i]);
		        		  write_unlock(&pfr->ring_rules_lock);
		        	  }

		        	  kfree(rules_to_delete);
		        	  rules_to_delete = NULL;
